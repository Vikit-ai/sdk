...
http_timeout = aiohttp.ClientTimeout(
    total=1500, connect=1500, sock_read=1500, sock_connect=1500
)
...
interesting the resulting music will be. Here is your prompt: '"""
    ...
    async def generate_video_async(
        self, prompt, model_provider: str, aspect_ratio=(16, 9)
    ):
        """
        ...
        if model_provider == "vikit":
            ...
            return await self.generate_video_from_image_stabilityai_async(
                prompt, aspect_ratio
            )
    ...
    async def generate_video_from_image_and_text_runway(
        ...
        if prompt.image is None:
            raise ValueError("Image needs to be provided for Runway model")
        try:
            logger.debug(
                f"Generating video from image prompt {prompt.image[:50]} and text {prompt.text}"
            )
    ...
    async def ask_gemini(
        ...
        """
        logger.debug(
            f"Galling Gemini model {gemini_version} with prompt {prompt} text {prompt.text}"
        )
        ...
        if prompt.video is not None and prompt.video.startswith("http"):
            ...
            if prompt.video.split(".") is not None and prompt.video.split(".")[
                -1
